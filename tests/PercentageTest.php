<?php

use ZeroBoiler\ValueObjects\ValueObjects\Percentage;

test('percentage can be created', function () {
    $percentage = new Percentage(50.0);

    expect($percentage->value)->toBe(50.0);
});

test('percentage must be between 0 and 100', function () {
    expect(fn () => new Percentage(-1.0))->toThrow(\Illuminate\Validation\ValidationException::class);
    expect(fn () => new Percentage(101.0))->toThrow(\Illuminate\Validation\ValidationException::class);
});

test('percentage can calculate of a number', function () {
    $percentage = new Percentage(25.0);

    expect($percentage->of(100))->toBe(25.0);
});

test('percentage can apply to an amount', function () {
    $percentage = new Percentage(20.0);

    expect($percentage->applyTo(50))->toBe(10.0);
});

test('percentage can be added', function () {
    $p1 = new Percentage(30.0);
    $p2 = new Percentage(20.0);

    $result = $p1->add($p2);

    expect($result->value)->toBe(50.0);
});

test('percentage addition clamps to 100', function () {
    $p1 = new Percentage(80.0);
    $p2 = new Percentage(50.0);

    $result = $p1->add($p2);

    expect($result->value)->toBe(100.0);
});

test('percentage can be subtracted', function () {
    $p1 = new Percentage(50.0);
    $p2 = new Percentage(20.0);

    $result = $p1->subtract($p2);

    expect($result->value)->toBe(30.0);
});

test('percentage subtraction clamps to 0', function () {
    $p1 = new Percentage(10.0);
    $p2 = new Percentage(30.0);

    $result = $p1->subtract($p2);

    expect($result->value)->toBe(0.0);
});

test('percentage can be multiplied', function () {
    $percentage = new Percentage(25.0);

    $result = $percentage->multiply(2.0);

    expect($result->value)->toBe(50.0);
});

test('percentage multiplication clamps to 100', function () {
    $percentage = new Percentage(75.0);

    $result = $percentage->multiply(2.0);

    expect($result->value)->toBe(100.0);
});

test('percentage can check if zero', function () {
    expect((new Percentage(0.0))->isZero())->toBeTrue()
        ->and((new Percentage(50.0))->isZero())->toBeFalse();
});

test('percentage can check if full', function () {
    expect((new Percentage(100.0))->isFull())->toBeTrue()
        ->and((new Percentage(50.0))->isFull())->toBeFalse();
});

test('percentage equals compares by value', function () {
    $p1 = new Percentage(50.0);
    $p2 = new Percentage(50.0);
    $p3 = new Percentage(25.0);

    expect($p1->equals($p2))->toBeTrue()
        ->and($p1->equals($p3))->toBeFalse();
});

test('percentage can be converted to string', function () {
    $percentage = new Percentage(50.0);

    expect((string) $percentage)->toBe('50%');
});

test('percentage with decimals shows decimals', function () {
    $percentage = new Percentage(33.333);

    expect((string) $percentage)->toBe('33.33%');
});

test('percentage can be serialized', function () {
    $percentage = new Percentage(50.0);

    expect($percentage->toArray())->toBe(['value' => 50.0]);
});